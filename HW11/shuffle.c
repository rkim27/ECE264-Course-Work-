...
void kelp(CardDeck leftDeck, CardDeck rightDeck, CardDeck deck, int lind, int Rind, int round);
...
{ 
	...
	//call helper with one less left card (lind + 1)
		kelp(leftDeck, rightDeck, deck, lind + 1, Rind, round);
	}
	if (Rind < rightDeck.size)
	{
		deck.cards[ind] = rightDeck.cards[Rind];
		kelp(leftDeck, rightDeck, deck, lind, Rind + 1, round);
	}
	//if (ind == (deck.size - 1))
	//{
	//printDeck(deck);
...
{ //Have a string to print order into, have two sets recursive structures that start at either left or right 
	...
	kelp(leftDeck, rightDeck, deck, 0, 0, round);
	//print order
}
//#endif
